...
import signal
import argparse
import unfold_circuit as unf
import most_similar_circuit as mSimCir
import extract_frame_time as ext_fr
import extract_bmc_engine as bmc
import re
import subprocess
TIMELIMIT = 1
total_time = 120
...
def terminate_process(signum, frame):
    ...
    for x, y in engine_sequence.items():
        print(f"{x}: {y}")
    print(f"\n{D}\n")
    print(f"Delay between actual termination time and expected termination time: {round(time.time() - end_time, 2)} secs.")
...
def run_engine(selected_engine, circuit, FLAGS):
    ...
    if selected_engine == "bmcJ":
        ...
        command = ["abc", f"-c read {circuit}; print_stats; &get; {selected_engine} {FLAGS}; print_stats"]
...
def process_circuits(start_time, total_time, cir_name, FRAME, end_time, engine_sequence, count, s, FLAGS, CURR_FRAME):
    ...
    selected_engine = bmc.extract_bmc_engine(engine)
    print(f"DEPTH ({FRAME}) : DeepGate2 execution + BMC engine selection : {round(time.time() - s, 2)} secs\n")
...
def run_f_mode(args):
    ...
    try:
        while time.time() - start_time <= total_time:
            s = time.time()
            print("============================================================================================================================================================================================================\n")
            if (CURR_FRAME != START_FRAME and START_FRAME != -1) or first_iter or START_FRAME != -1:
                first_iter = False
